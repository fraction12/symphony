defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server forwards model and effort turn overrides" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-turn-overrides-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-turn-overrides.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-turn-overrides.trace}"
      count=0
      ...
      issue = %Issue{
        id: "issue-turn-overrides",
        identifier: "MT-1002",
        title: "Validate turn overrides",
        description: "Ensure runtime forwards requested execution settings.",
        state: "In Progress",
      ...
      try do
        assert {:ok, _result} =
                 AppServer.run_turn(session, "Validate turn overrides", issue,
                   model: "gpt-custom",
                   effort: "high"
                 )
      after
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 |> then(fn payload ->
                   payload["method"] == "turn/start" &&
                     get_in(payload, ["params", "model"]) == "gpt-custom" &&
